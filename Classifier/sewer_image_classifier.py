...
Original file is located at the following URL:
https://colab.research.google.com/drive/1nHuLYF6KRchLrNmTF7ip7xFYrg6rOCue
'''
# pytorch_lightning abstracts the training modules
import pandas as pd
import numpy as np
import torchvision
...
class SewerMLDataset(Dataset):
    def __init__(self, data_root: Path, label_path: Path, split = "Train", transform = None):
        ...
        ''' Taking a subset of the defect types to align with Crozier's data domain
...
# Generic class to use with any candidate model
class SewerMLClassifier(pl.LightningModule):
    def __init__(self, model, lr):
        super().__init__()
        self.model = model
        self.loss = nn.CrossEntropyLoss() # softmax layer not required in model
...
def main(args):
    ...
    # Initialize the Weight Transforms (should go in dataset/dataloader)
